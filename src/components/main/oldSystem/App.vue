...
export default {
  ...
  methods: {
    ...
    changeContent(url) {
      ...
      // }
      if (this.url.indexOf('XXX') > 0) {
        this.url = url.split('userName')[0] + 'userName=' + sessionStorage.getItem('newLname');
      } else {
        this.url = url.split('userName')[0] + 'userName=' + sessionStorage.getItem('LOGIN_NAME');
      }
      // console.log(this.url)
    },
    loadMenu() {
      this.$http.get('api/menu/sheme/ref/son?schemeId=' + sessionStorage.getItem('SCHEME_ID') + '&menuId=64')
        .then((res) => {
          this.spas = res.data.data
          if (this.spas[0].MENU_URL.indexOf('XXX') > 0) {
            this.url = this.spas[0].MENU_URL.split('userName')[0] + 'userName=' + sessionStorage.getItem('newLname');
          } else {
            this.url = this.spas[0].MENU_URL.split('userName')[0] + 'userName=' + sessionStorage.getItem('LOGIN_NAME');
          }
